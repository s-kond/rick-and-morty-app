...
function buildAPI({ search, filter }) {
  ...
  if (search2 === "") {
    return `https://rickandmortyapi.com/api/character/${
      filterAPI ? "?status=" + filterAPI : ""
    }`;
  }
  return `https://rickandmortyapi.com/api/character/?name=${search2}${
    filterAPI ? "&status=" + filterAPI : ""
  }`;
}
...
//um die kostenlose API und das Datenvolumen der User nicht überzustrapazieren wurde diese Funktion deaktiviert
...
searchForm.addEventListener("submit", (event) => {
  ...
  const apiURL = buildAPI(formdata);
